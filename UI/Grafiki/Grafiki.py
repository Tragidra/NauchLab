...
from PyQt5.QtWidgets import QApplication, QMainWindow, QCheckBox, QComboBox, QGridLayout, QHBoxLayout, \
    QLabel, QSizePolicy, QWidget, QDialog
...
class ThemeWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        loadUi("Grafiki/Diog.ui", self)
        self.pushg.clicked.connect(self.diog)
    def diog(self):
        self.m_charts = []
        self.m_listCount = 3
        self.m_valueCount = int(self.le1.text())
        tempmass = self.le2.text()
        tempmass = tempmass.split(',')
        self.m_dataTable = self.generateRandomData(self.m_valueCount,tempmass)
        #print(self.m_dataTable)
    ...
    def generateRandomData(self,valueCount,mass1):
        dataTable = []
        massY = []
        massX = []
        yValue = 0.0
        ...
        for i in range(valueCount*2):
            if i%2 == 0:
                massX[k] = float(mass1[i])
                k = k + 1
            else:
                massY[k2] = float(mass1[i])
                k2 = k2 + 1
        print(massY)
        for i in range(0,1):
            dataList = []
            for j in range(valueCount):
                #yValue += massY[j] / f_valueCount
                print(j)
                value = QPointF(
                    massX[j],
                    massY[j])
